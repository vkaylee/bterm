...
- **Vietnamese IME Support:** Giải quyết vấn đề không thể gõ tiếng Việt (Telex/VNI) trên mobile. Khi phát hiện thiết bị di động, ứng dụng kích hoạt `screenReaderMode` của Xterm.js và cấu hình các thuộc tính của hidden textarea (`autocorrect="on"`, `spellcheck="true"`, `inputmode="text"`). Điều này cho phép bàn phím hệ thống thực hiện quá trình xử lý ký tự (composition) và gợi ý từ một cách chính xác trước khi gửi dữ liệu vào terminal.
